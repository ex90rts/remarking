...
Remarker 是本地优先工具：足迹、划线、生词和设置默认保存在浏览器 IndexedDB 中。单词查词记录和结果统一保存在生词数据中；文本翻译只即时展示，不做持久化保存。LLM API key 只由扩展后台 service worker 读取和使用，不会写入页面 DOM。
...
Remarker 默认把应用数据保存在浏览器本地 IndexedDB 中。用户可以按需导出足迹、划线、生词和部分应用数据。
